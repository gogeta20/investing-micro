import HttpClientRust from '@/core/http/HttpClientRust';
...
class RepositoryDjangoGet extends BaseRepository implements IRepository {
  ...
  async Api(params) : Promise<ResponseData> {
    try {
      const responseBasic = await HttpClientRust.get<ResponseBasic>(this.path, params);
...
export default RepositoryDjangoGet;
