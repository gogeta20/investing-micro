...
class RepositorRustPost extends BaseRepository implements IRepository {
  ...
  async Api(params) : Promise<ResponseData> {
    try {
      const responseBasic = await HttpClientRust.post<ResponseBasic>(this.path, params);
...
export default RepositorRustPost;
